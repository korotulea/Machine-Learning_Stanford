...
%
X = [ones(m, 1) X];
h_func1 = sigmoid(X*Theta1');
a2 = [ones(m, 1) h_func1];
a3 = sigmoid(a2*Theta2');
yy = zeros(num_labels, size(y,1)); 
for c = 1:num_labels 
    yy(c,:) = (y == c);       
end
J1 = - log(a3) .* yy' ;
...
th1_sq = Theta1(:, 2:end).^2;
th2_sq = Theta2(:, 2:end).^2;
reg_cost = lambda / (2 * m) * (sum(th1_sq(:))+sum(th2_sq(:)));
